import { useFetchCreateCategory } from '@/hooks/apis/category'
...
export default function CreateCategoryModal({ open, onClose }: CreateCategoryModalProps) {
  ...
  const { mutate: createCategory, isPending: isCreateCategoryPending } = useFetchCreateCategory({
    onSuccess: () => {
      ...
      >
        ...
        <form onSubmit={handleSubmit}>
          ...
          <Box sx={{ mt: 3, display: 'flex', justifyContent: 'flex-end', gap: 2 }}>
            <Button variant='outlined' onClick={handleCancel} disabled={isCreateCategoryPending}>
              Hủy
            </Button>
            <Button type='submit' variant='contained' disabled={isCreateCategoryPending}>
              TẠO
            </Button>
          </Box>
        </form>
