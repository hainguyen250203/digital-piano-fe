...
import { useFetchProductListBySubCategory } from '@/hooks/apis/product'
import { useProductFilters } from '@/hooks/useProductFilters'
import { ProductListBySubCategory } from '@/types/product.type'
...
const usePagination = (items: ProductListBySubCategory['products'] = [], itemsPerPage: number = 12) => {
...
export default function ProductsPage({ params }: ProductsPageProps) {
  const { id } = use(params)
  const { data: subCategory, isLoading, error } = useFetchProductListBySubCategory(id)
  const [searchQuery, setSearchQuery] = useState('')
  const { setFilters, filteredAndSortedProducts } = useProductFilters(subCategory?.data?.products)
  ...
  return (
    <Box sx={{ py: 4 }}>
      <Breadcrumbs separator={<NavigateNext fontSize='small' />} aria-label='breadcrumb' sx={{ mb: 2 }}>
        ...
        <Link href={`/products/category/${subCategory?.data?.category?.id}`} style={{ textDecoration: 'none', color: 'inherit' }}>
          <Typography color='text.secondary'>{subCategory?.data?.category?.name}</Typography>
        </Link>
        <Typography color='text.primary'>{subCategory?.data?.name}</Typography>
      </Breadcrumbs>
