...
import { useFetchProductListByProductType } from '@/hooks/apis/product'
import { useProductFilters } from '@/hooks/useProductFilters'
import { ProductListByProductType } from '@/types/product.type'
import { Clear, FilterList, NavigateNext, Search } from '@mui/icons-material'
import { alpha, Box, Breadcrumbs, Button, IconButton, InputBase, Pagination, Paper, Skeleton, Typography } from '@mui/material'
...
const usePagination = (items: ProductListByProductType['products'] = [], itemsPerPage: number = 12) => {
...
export default function ProductTypePage({ params }: ProductTypePageProps) {
  const { id } = use(params)
  const { data: productType, isLoading, error } = useFetchProductListByProductType(id)
  const [searchQuery, setSearchQuery] = useState('')
  const { setFilters, filteredAndSortedProducts } = useProductFilters(productType?.data?.products)
  const { page, totalPages, paginatedItems, handlePageChange } = usePagination(filteredAndSortedProducts?.filter(product => product.name.toLowerCase().includes(searchQuery.toLowerCase())))
  ...
  return (
    <Box sx={{ py: 4 }}>
      <Breadcrumbs separator={<NavigateNext fontSize='small' />} aria-label='breadcrumb' sx={{ mb: 2 }}>
        ...
        <Link href={`/products/sub-category/${productType?.data?.subCategory?.id}`} style={{ textDecoration: 'none', color: 'inherit' }}>
          <Typography color='text.secondary'>{productType?.data?.subCategory?.name}</Typography>
        </Link>
        <Typography color='text.primary'>{productType?.data?.name}</Typography>
      </Breadcrumbs>
      ...
      <ProductList products={paginatedItems} error={error} />
